...
const { FrontEnd } = require("../config/app.config")
...
class AuthController {
    ...
    activateUser = async (req, res, next) => {
        try {
            ...
            if (!userDetail) {
                res.redirect(`${FrontEnd.url}?status=error&msg=Token does not associate with any user.`);
                // we are not allowed to throw any error here
            } else {
                const today = Date.now();
                if (today > userDetail.expiryTime) {
                    res.redirect(`${FrontEnd.url}?expired=1&status=error&msg=Activation link is expired, please request another link`)
                    // we are not allowed to throw any error here.
                } else {
                    ...
                    res.redirect(`${FrontEnd.url}?status=activationSuccess&msg=Your account has been activated successfully`)
                }
            }
        }
        catch (exception) {
            res.redirect(`${FrontEnd.url}?status=error&msg=Activation failed.`);
        }
    }
